...
newsRouter
    ...
    .post(async (req, res) => {
        try {
            const { topic_id, news_title, news_body, tags, news_status_id } =
                req.body;
            let badInput = "";
            if (!topic_id || typeof topic_id !== typeof 1) {
                badInput += "`topic_id` is mandatory and must be an integer";
            } else if (!news_title) {
                badInput += "`news_title` is mandatory";
            } else if (!news_body) {
                badInput += "`news_body` is mandatory";
            } else if (!tags || tags.length === 0) {
                badInput += "`tags` is array of string and must be more than 0";
            } else if (!news_status_id || typeof news_status_id !== typeof 1) {
                badInput += "`news_status_id` is mandatory must be an integer";
            }
